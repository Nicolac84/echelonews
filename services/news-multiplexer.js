...
class NewsMultiplexer {
  ...
  async setup() {
    try {
      ...
      this.channel.consume(this.queueName, async function(msg) {
        try {
...
class NewsMultiplexerClient extends EventEmitter {
  ...
  async setup() {
    ...
    try {
      ...
      this.channel.consume(this.responseQueue.queue, function(msg) {
        log.info(`Received response with correlation ID ${msg.properties.correlationId}`)
        this.emit(`rpc${msg.properties.correlationId}`, JSON.parse(msg.content.toString()))
      }.bind(this))
    } catch (err) {
      log.error(err)
      throw err
...
if (require.main === module) {
  ...
  if (!broker) {
    log.fatal('AMQP broker is not defined. You must properly define the AMQP_BROKER environment variable')
    process.exit(1)
  }
